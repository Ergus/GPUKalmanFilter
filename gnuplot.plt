...
### Arrays
serials = "Bad Good Bad_Good OMP"
titles1 = "AOS SOA SOA+conversion OpenMP"
...
plot for [i=1:3] sprintf(plotCPU,word(serials,i),fbadgood) t word(titles1,i) w linespoints pt 7 lw 0.5 ps 0.75

## Test graph using OpenMP, not to include in the report
...
### Only for Cuda.
set title "Time vs Number of Tracks from 1 to 300 pointer vs loop in Cuda"
...
### The comparison time betwen loop copy and pinter access to global memory
### Only for OpenCl.
set title "Time vs Number of Tracks from 1 to 300 pointer vs loop in OpenCl"
...
plot for [i=2:3] sprintf(plotSU,word(serials,i),fbadgood) t word(titles1,i) w linespoints pt 7 lw 0.5 ps 0.75
...
### OpenMP speedup ############
set output '|epstopdf --filter --outfile=Graphs/SU_OMP_Code.pdf'
plot for [i=2:words(serials)] sprintf(plotSU,word(serials,i),fomp) t word(titles1,i) w linespoints pt 7 lw 0.5 ps 0.75,\
     for [i=1:2] sprintf(plotSU2,word(GPUs,i),fcpu) t word(GPUs,i) w linespoints pt 7 lw 0.5 ps 0.75